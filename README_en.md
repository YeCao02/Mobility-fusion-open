...
The GBA sample contains about 1000 uuid-days per mainland Greater Bay Area city, 9000 uuid-days in total. The Shanghai page is not a standalone quality-validation workflow. It samples raw `residence_*` gzip files from `S:\GEO BIG data\Shanghai-2026`, rewrites them into the original pipeline's `unified_points` raw-input format, runs the same local `FULL_OD` production pipeline, and then converts those outputs into the same public demo `manifest + processed/raw` shard contract.
