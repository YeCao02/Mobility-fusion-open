...
本仓库是 **Mobility Fusion** 的公开展示版本，用于说明 PioneerData 多源手机 SDK 原始事件如何被清洗、融合并构造为可审计的用户日内 `FULL_OD` 链。公开仓库只包含框架说明、抽样 demo 数据和浏览器，不包含全量原始数据和私有生产环境配置。
...
[https://yecao02.github.io/Mobility-fusion-open/demo/](https://yecao02.github.io/Mobility-fusion-open/demo/)
...
当前公开 demo 使用 `FULL_OD` 口径：在完成原始事件标准化、H3 编码、局部支撑校验、近邻吸收、防抖、速度与上下文检查后，保留日内连续的 `DAY_START / STAY / STOP / DAY_END` 节点，并由相邻节点生成 OD 段。
...
- `S`：标准化事件未成为最终代表 H3 节点，通常表示被邻近代表节点吸收、压制或未匹配。
- `D`：原始阶段已有明确异常或硬剔除标记。
...
│  └─ full_od_strategy_framework_public_zh.svg
├─ demo/
│  ├─ index.html
│  └─ data/
│     ├─ manifest.json
